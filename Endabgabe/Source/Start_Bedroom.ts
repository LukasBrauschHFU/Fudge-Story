namespace Template {
  export async function Start_Bedroom(): ƒS.SceneReturn {
    ...
    ƒS.Sound.play(sound.creak, 1);
    await ƒS.update(5);
    ƒS.Sound.fade(sound.darkTheme, 0.2, 0.1, true);
    //Noises Here
    await ƒS.update(1);
    ...
    switch (lightDecisionElement) {
      case lightDecision.turnOnLight:
        ƒS.Sound.fade(sound.darkTheme, 0, 0.1, false);
    ...
    switch (lightDecisionElement2) {
      ...
      case lightDecision2.askFriendly:
        ...
        await ƒS.Speech.tell(characters.Protagonist, "Ah, yeah, I kinda remember seeing you there. Weren´t you that girl who drank a whole bottle of *brand censored* vodka at the campus party and "),
          await ƒS.Speech.tell(characters.Protagonist, "tried to fight a fly girl because she stole one of your pizza slices ?"),
          await ƒS.Character.hide(characters.Mothrin);
        await ƒS.Character.show(characters.Mothrin, characters.Mothrin.pose.sad, ƒS.positionPercent(105, 150));
        await ƒS.update(1);
    ...
    await ƒS.Character.hide(characters.Mothrin);
    await ƒS.update(1);
    await ƒS.Speech.tell(characters.Narrator, "As Mothrin leaves, you see some scales falling from her wings. You pick them up and store them in a jar");
    ƒS.Inventory.add(items.Mothdust);
    await ƒS.Character.hide(characters.Protagonist);
