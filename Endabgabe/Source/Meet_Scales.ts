namespace Template {
  export async function Meet_Scales(): ƒS.SceneReturn {
    ...
    await ƒS.update(1);
    ƒS.Sound.fade(sound.walking, 0.2, 2, true);
    await ƒS.update(5);
    ƒS.Sound.fade(sound.walking, 0, 1, false);
    ...
    switch (importantDecisionElement) {
      ...
      case importantDecision.stay:
        ...
        await ƒS.update(1);
        //working noises here
        ƒS.Sound.fade(sound.clank, 0.2, 2, true);
        await ƒS.update(5);
        ƒS.Sound.fade(sound.clank, 0, 1, false);
        await ƒS.Location.show(locations.HFU_Room);
        await ƒS.Character.show(characters.Protagonist, characters.Protagonist.pose.normal, ƒS.positionPercent(29, 150));
        await ƒS.Character.show(characters.Scales, characters.Scales.pose.normal, ƒS.positionPercent(102, 150));
        await ƒS.update(1);
        ...
        ƒS.Speech.clear();
        await ƒS.update(1);
        dataForSave.ScalesAffection.score = dataForSave.ScalesAffection.score + 1;
